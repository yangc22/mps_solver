...
% for versions after MATLAB 2018 with 'str2sym' to create symbolic variables
    ...
    if (size(tline, 2) ~= 0 && tline(1) ~= '#')
        ...
        if (TF)
            ...
            switch exp
                case 'StateVariables'
                    fprintf(1,'%s\n',tline);
                    sv = transpose(str2sym(char(C(2))));
                    nx = size(sv, 1);
                case 'ControlVariables'
                    fprintf(1,'%s\n',tline);
                    cv = transpose(str2sym(char(C(2))));
                    nu = size(cv, 1);
                case 'ParameterVariables'
                    fprintf(1,'%s\n',tline);
                    pv = transpose(str2sym(char(C(2))));
                    nw = size(pv, 1);
                ...
                case 'StateEstimate'
                    fprintf(1,'%s\n',tline);
                    state_estimate = str2sym(char(C(2)));
                    flag_se = 1;
                case 'ControlEstimate'
                    fprintf(1,'%s\n',tline);
                    control_estimate = str2sym(char(C(2)));
                    flag_ce = 1;
                case 'ParameterEstimate'
                    fprintf(1,'%s\n',tline);
                    parameter_estimate = str2sym(char(C(2)));
                    flag_pe = 1;
                case 'InitialTime'
                    fprintf(1,'%s\n',tline);
                    t0 = str2sym(char(C(2)));
